...
With file attachments (to a specific Telegram topic):
...
- `--message-thread-id`: (Optional, Telegram only) Forum topic ID to send the message to. Without this, messages go to the main chat instead of the correct topic thread.
