...
async function sendMessage(argv) {
    ...
    const messageThreadId = args['message-thread-id'] ? parseInt(args['message-thread-id'], 10) : undefined;
    ...
    if (missing.length > 0) {
        ...
        console.error('Usage: send --channel <ch> --sender-id <id> --sender <name> --message <msg> [--message-thread-id <id>]');
        process.exit(1);
    }
